...
class DemoRestApiItem(APIView):
    ...
    def put(self, request, item_id):
        ...
        if 'id' not in data:
            return Response({'error': 'Falta el campo id en el cuerpo.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if 'name' not in data or 'email' not in data:
            return Response({'error': 'Faltan campos requeridos para reemplazo completo.'}, status=status.HTTP_400_BAD_REQUEST)
